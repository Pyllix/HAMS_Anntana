...
### การไม่อนุมัติ / ปฏิเสธขั้นตอน (Rejection & Re-diagnosis Flow)
- **เมื่อพัสดุปฏิเสธการเบิกอะไหล่ / ส่งซ่อมนอก (`PATCH /api/v1/repairs/:id/steps/reject`):**
  - อนุญาตเฉพาะ Step 5 ของแทร็ก `WITH_PARTS` หรือ `OUTSOURCE` ดำเนินการโดย `PARCEL_STAFF` (หรือ `MANAGER`)
  - ระบบปรับสถานะงานซ่อมเป็น **`IN_PROGRESS`** (ช่างกำลังดำเนินการซ่อม) เพื่อให้คงอยู่ในความรับผิดชอบของช่างคนเดิม ไม่หลุดกลับไปคิวงานใหม่ของหัวหน้าช่าง
  - รายการขอเบิกอะไหล่ที่รอตัดสต็อก (`PENDING_WITHDRAW`) ในใบงานนั้นจะถูกลบทิ้ง
  - ระบบบันทึกข้อความเหตุผล `[ไม่อนุมัติ] ${reason}` ลงใน Step 5
  - ระบบจะบล็อกการกดอัปเดต Step อื่นๆ ทั้งหมด (`updateStepProgress`) จนกว่าช่างจะกดบันทึกผลการวินิจฉัยและวางแผนใหม่ (`PATCH /api/v1/repairs/:id/diagnose`) เพื่อจัดชุดสเต็ปใหม่ตามแผนที่ปรับปรุง
...
* **ขอบเขตสิทธิ์ (Role Scoping):** บังคับใช้ 2FA เฉพาะ **`ADMIN`**, **`PARCEL_STAFF`**, และ **`ASSET_CENTER_STAFF`** เท่านั้น ผู้ใช้ใน Role อื่นไม่ต้องเปิดใช้ 2FA และยังคงใช้การล็อกอินแบบปกติ
* **Mandatory 2FA Enrollment Gate:** ผู้ใช้ใน Role ที่ถูกบังคับต้องลงทะเบียน Authenticator และยืนยัน TOTP สำเร็จก่อน จึงจะได้รับ Session ปกติและเข้าใช้งานฟังก์ชันงานของระบบได้
* **การเปิดใช้บัญชี:** Admin สามารถสร้างบัญชีใน Role ที่ถูกบังคับได้ แต่บัญชีจะเข้าได้เฉพาะขั้นตอนลงทะเบียน 2FA จนกว่าเจ้าของบัญชีจะผูก Authenticator และยืนยัน TOTP ด้วยตนเองสำเร็จ โดย Admin ต้องไม่เห็นหรือเก็บ Secret Key แทนผู้ใช้
...
* **Recovery Codes:** เมื่อ Enrollment สำเร็จ ระบบสร้าง Recovery Codes จำนวน 10 รหัสให้ผู้ใช้เก็บไว้ รหัสแต่ละรายการใช้ยืนยันตัวตนแทน TOTP ได้เพียงครั้งเดียวและต้องจัดเก็บในระบบแบบเข้ารหัส ผู้ใช้ต้องยืนยันว่าเก็บรหัสแล้วก่อนเปิดใช้งานบัญชีเสร็จสมบูรณ์
* **Admin-assisted 2FA Reset:** เมื่อผู้ใช้สูญเสียทั้ง Authenticator และ Recovery Codes ต้องให้ `ADMIN` คนอื่นตรวจสอบตัวตนนอกระบบก่อนรีเซ็ต การรีเซ็ตต้องเพิกถอน Session ทั้งหมด ลบ Secret Key และ Recovery Codes ชุดเดิม บันทึกผู้ดำเนินการ เวลา และเหตุผลใน Audit Log แล้วบังคับให้เจ้าของบัญชีทำ Enrollment ใหม่ก่อนกลับเข้าใช้ฟังก์ชันงาน
* **Admin Recovery Quorum:** ระบบต้องมี `ADMIN` ที่ยัง Active และผ่าน 2FA Enrollment แล้วอย่างน้อย 2 บัญชีเสมอ ห้ามลบ ปิดใช้งาน หรือลด Role ของ Admin หากทำให้จำนวนผู้ที่พร้อมกู้บัญชีต่ำกว่าเกณฑ์นี้
* **Trusted Browser:** ผู้ใช้เลือกเชื่อถือ Browser บนเครื่องประจำได้ครั้งละ 14 วันแบบ Absolute Expiry โดยตัวเลือกต้องไม่ถูกเลือกไว้ล่วงหน้า Trust ต้องผูกกับทั้งบัญชีผู้ใช้และ Browser ห้ามใช้ข้ามบัญชี และห้ามเปิดใช้บนเครื่องส่วนกลางหรือเครื่องที่มีการสลับใช้งานหลายบัญชี เมื่อพบบัญชีอื่นล็อกอินผ่าน Browser เดียวกัน ระบบต้องเพิกถอน Trust เดิมและถือว่า Browser นั้นเป็นเครื่องที่มีการใช้งานร่วมกัน
* **2FA Lifecycle:** ผู้ใช้ใน Role ที่ถูกบังคับไม่มีสิทธิ์ปิด 2FA มีได้เฉพาะการเปลี่ยน Authenticator หลังยืนยันรหัสผ่านและ TOTP ปัจจุบัน หรือการรีเซ็ต 2FA ที่เพิกถอน Session เดิมและนำบัญชีกลับเข้า Mandatory 2FA Enrollment Gate
* **Step-up Authentication:** แม้ Browser ยัง Trusted ระบบต้องขอ TOTP ปัจจุบันอีกครั้งก่อนเปลี่ยนหรือรีเซ็ต 2FA สร้าง Recovery Codes ชุดใหม่ ให้ `ADMIN` รีเซ็ตรหัสผ่านของผู้ใช้อื่น หรือเปลี่ยน Role ของบัญชีเป็น/ออกจาก `ADMIN`
* **Step-up Window:** เมื่อ `ADMIN` ยืนยัน Step-up TOTP สำเร็จ สิทธิ์ยกระดับมีผล 5 นาทีและผูกกับ Admin และ Session ปัจจุบัน ภายในช่วงนี้สามารถรีเซ็ตรหัสผ่านหลายบัญชีได้โดยไม่ต้องกรอก TOTP ซ้ำ สิทธิ์ต้องสิ้นสุดทันทีเมื่อออกจากระบบ Session ถูกเพิกถอน หรือ Admin เปลี่ยนรหัสผ่าน/2FA
* **2FA Verification Lockout:** ระบบนับการยืนยัน TOTP และ Recovery Code ที่ผิดติดต่อกันรวมกัน เมื่อผิดครบ 5 ครั้งให้ระงับการยืนยัน 2FA ของบัญชีนั้น 10 นาทีและบันทึก Audit Log การยืนยันสำเร็จต้องล้างตัวนับ โดยการระงับนี้ไม่ใช่การปิดบัญชีถาวร
* **TOTP Parameters:** ใช้รหัส 6 หลัก รอบเวลา 30 วินาที และยอมรับรหัสจากช่วงก่อนหน้า ปัจจุบัน หรือช่วงถัดไป (`window = 1`) เพื่อรองรับเวลาระหว่างอุปกรณ์กับ Server ที่คลาดประมาณ ±30 วินาที
