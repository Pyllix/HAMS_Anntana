...
export class ForecastService {
  ...
  async getExpenseForecast(
    ...
    try {
      if (!sectionId || sectionId === 'all') {
        rows = await this.prisma.$queryRaw<RowType[]>`
          WITH combined_expenses AS (
            ...
            LEFT JOIN asset a ON rj.asset_id = a.asset_id
            LEFT JOIN sections s ON COALESCE(a.section_id, rj.section_id) = s.section_id
            WHERE st.txn_type = 'WITHDRAW' 
              AND s.section_id IS NOT NULL
              AND st.txn_date >= '2023-01-01'
      ...
      this.logger.error(`Error querying combined expenses: ${err.message}`);
    }
    ...
    // 3.1 ดึงข้อมูลครุภัณฑ์ที่กำลังใช้งานจริงและประเมินอายุขัยมาตรฐาน (asset_type.useful_life)
    type AssetExpiryRow = {
      asset_id: string;
      name: string;
      model: string;
    ...
    try {
      const assetRows = (!sectionId || sectionId === 'all')
        ? await this.prisma.$queryRaw<AssetExpiryRow[]>`
            SELECT 
              ...
              s.section_id,
              COALESCE(NULLIF(s.name, ''), s.section_code) AS section_name,
              ((a.receive_date + (at.useful_life || ' years')::interval) <= CURRENT_DATE) AS is_exceeded
            FROM asset a
      ...
      const criticalAssets: ExpiringAssetItem[] = assetRows
        .filter((r) => r.is_exceeded || r.price > 100000)
        .slice(0, 15)
        .map((r) => ({
          id: r.asset_id,
          name: r.name,
          model: r.model,
          noid: r.noid || undefined,
          asset_type: r.type_name,
          useful_life: r.useful_life,
          receive_date: r.receive_date,
          expiry_date: r.expiry_date,
          age_years: r.age_years,
          price: r.price,
          status: r.is_exceeded ? 'EXCEEDED' : 'EXPIRING_SOON',
      ...
      this.logger.warn(`Error evaluating asset lifespan: ${err.message}`);
    }
