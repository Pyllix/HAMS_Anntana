...
- ❌ **Do not let technicians directly select external vendors for `OUTSOURCE` repairs**: Technicians diagnose and mark `stepActionType: "OUTSOURCE"`, but `PARCEL_STAFF` is responsible for selecting the vendor (`company_id`), managing quotations/POs, and coordinating with the vendor at Step 5. When the machine returns, `MAINTENANCE_STAFF` executes Step 6 ("รับเครื่องคืนและทดสอบ") to test the machine and record the bill number and actual repair cost.
