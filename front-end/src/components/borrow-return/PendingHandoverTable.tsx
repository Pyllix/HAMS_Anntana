...
function HandoverAction({ transaction }: { transaction: BorrowHistory }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 rounded-lg bg-cyan-600 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition-colors hover:bg-cyan-700 disabled:cursor-not-allowed disabled:bg-cyan-300"
    >
      {isSubmitting ? (
...
const columns: Array<ColumnDef<typeof features, BorrowHistory>> = [
  ...
  },
  {
    id: "actions",
    header: "การดำเนินการ",
    cell: (info) => <HandoverAction transaction={info.row.original} />,
  },
];

export default function PendingHandoverTable() {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      <div className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-slate-200 bg-cyan-50/60">
        <span className="flex h-2 w-2 rounded-full bg-cyan-500" />
        <h2 className="text-sm font-semibold text-slate-800">
          รายการรอส่งมอบครุภัณฑ์
        </h2>
        <span className="ml-1 inline-flex items-center justify-center min-w-[22px] h-[22px] px-1.5 rounded-full bg-cyan-500 text-white text-xs font-semibold">
          {pendingItems.length}
        </span>
      </div>
