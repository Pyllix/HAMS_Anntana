  ...
  },
  {
    ...
    cell: (info) => {
      ...
      return (
        <button
          type="button"
          onClick={() => useCompleteReturnModalStore.getState().openForm(row)}
...
export default function PendingCompleteReturnTable() {
  ...
  const table = useTable({
    key: "pending-complete-return-table",
    features,
    columns,
    data: pendingItems,
    initialState: {
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      <div className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-slate-200 bg-emerald-50/60">
        <span className="flex h-2 w-2 rounded-full bg-emerald-500" />
        <h2 className="text-sm font-semibold text-slate-800">
          รายการรอตรวจรับเข้าคลัง
        </h2>
        <span className="ml-1 inline-flex items-center justify-center min-w-[22px] h-[22px] px-1.5 rounded-full bg-emerald-500 text-white text-xs font-semibold">
          {pendingItems.length}
        </span>
      </div>
