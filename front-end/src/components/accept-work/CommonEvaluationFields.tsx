      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="relative" ref={causeDropdownRef}>
          ...
          {isCauseOpen && !readOnly && (
            <div className="absolute z-20 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg max-h-[216px] overflow-y-auto py-1">
              ...
              {causes.map((cause) => {
                const isSelected = String(cause.id) === String(formState.causeId);
                return (
                  <div
                    key={cause.id}
                    onClick={() => {
        ...
        <div>
          ...
          <div className="flex items-center gap-4 h-9">
            <label className={`flex items-center gap-1.5 text-xs text-slate-600 ${readOnly ? "cursor-not-allowed" : "cursor-pointer"}`}>
              <input
                type="radio"
                name="isRepeatRepair"
                disabled={readOnly}
